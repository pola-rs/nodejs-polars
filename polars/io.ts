...
import {JsonScanOptions, ReadParquetOptions, ScanParquetOptions} from "./types";
...
export function readCSV(pathOrBody, options?) {
  ...
  if (Buffer.isBuffer(pathOrBody)) {
    return _DataFrame(
      pli.readCsv(pathOrBody, { ...readCsvDefaultOptions, ...options }),
    );
  }
  if (typeof pathOrBody === "string") {
    ...
    if (inline) {
      ...
      return _DataFrame(
        pli.readCsv(buf, { ...readCsvDefaultOptions, ...options }),
      );
    } else {
      return _DataFrame(
        pli.readCsv(pathOrBody, { ...readCsvDefaultOptions, ...options }),
 ...
 * @param options - Options for reading parquet files
 * @param options.numRows Stop reading from parquet file after reading ``numRows``.
 * @param options.cache Cache the result after reading.
