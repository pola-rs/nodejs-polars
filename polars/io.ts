 ...
 * It uses `fastexcel` <https://github.com/ToucanToco/fastexcel> library to bind the Rust-based Calamine parser.
 * 
 * @param source - Path(s) to a file or a file-like object 
 * @param options.idxOrName - Sheet index or name, Default index = 0
 * @param options.sheetId - Sheet number(s) to convert (set `0` to load all sheets as DataFrames) and
        return a `{sheetname:frame,}` dict. (Defaults to `1` if neither this nor
        `sheetName` are specified). Can also take a sequence of sheet numbers.
 * @param options.sheetName - Sheet name(s) to convert; cannot be used in conjunction with `sheet_id`. If
        more than one is given then a `{sheetname:frame,}` dict is returned.
 * @param options.tableName - Name of a specific table to read; note that table names are unique across
 ...
 * @param options.hasHeader - Indicate if the first row of the table data is a header or not. If False,
        column names will be autogenerated in the following format: `__UNNAMED__x`, with
        `x` being an enumeration over every column in the dataset, starting at 1.
 * @param options.nRows - Stop reading from Excel file after reading `nRows`.
 * @param options.skipRows - Start reading after `skipRows` rows.
 * @param options.schemaSampleRows - The maximum number of rows to scan for schema inference. If not set, 
        the full data may be scanned *(this can be slow)*. This parameter only applies if
        the input data is a sequence or generator of rows; other input is read as-is.
 */

export function readExcel(
  source: string,
  options?: Partial<ReadExcelOptions>,
): DataFrame;
export function readExcel(source, options?) {
  options = { ...options };
  options.idxOrName =
    options.idxOrName ?? options.sheetId ?? options.sheetName ?? 0;
  options.headerRow = options.hasHeader === false ? undefined : 0;
  return _DataFrame(pli.readExcel(source, options));
}
