...
impl JsExpr {
    ...
    #[napi(catch_unwind)]
    pub fn dt_convert_time_zone(&self, time_zone: String) -> JsExpr {
        let time_zone = TimeZone::opt_try_new(Some(PlSmallStr::from(time_zone))).map_err(JsPolarsErr::from).unwrap_or(Some(TimeZone::UTC)).unwrap();
        self
            .inner
            .clone()
            .dt()
    ...
    #[napi(catch_unwind)]
    pub fn dt_replace_time_zone(&self, time_zone: Option<String>, ambiguous: &JsExpr, non_existent: Wrap<NonExistent>) -> JsExpr {
        let time_zone: Option<TimeZone> = TimeZone::opt_try_new(time_zone.map(PlSmallStr::from_string)).map_err(JsPolarsErr::from).unwrap_or(Some(TimeZone::UTC));
        self
            .inner
            .clone()
            .dt()
