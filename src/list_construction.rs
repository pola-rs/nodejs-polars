...
pub fn js_arr_to_list(name: &str, arr: &Array, dtype: &DataType) -> napi::Result<Series> {
    ...
    let s = match dtype {
        DataType::Int8 => build_list_with_downcast!(PlSmallStr::from_str(name), arr, i8, DataType::Int8, Int8Type),
        DataType::UInt8 => build_list_with_downcast!(PlSmallStr::from_str(name), arr, u8, DataType::UInt8, UInt8Type),
        DataType::Int16 => build_list_with_downcast!(PlSmallStr::from_str(name), arr, i16, DataType::Int16, Int16Type),
        DataType::UInt16 => build_list_with_downcast!(PlSmallStr::from_str(name), arr, u16, DataType::UInt16, UInt16Type),
        DataType::Int32 => typed_option_or_null!(PlSmallStr::from_str(name), arr, i32, DataType::Int32, Int32Type),
        DataType::UInt32 => typed_option_or_null!(PlSmallStr::from_str(name), arr, u32, DataType::UInt32, UInt32Type),
        DataType::Float32 => {
            build_list_with_downcast!(PlSmallStr::from_str(name), arr, f32, DataType::Float32, Float32Type)
        }
        DataType::Int64 => typed_option_or_null!(PlSmallStr::from_str(name), arr, i64, DataType::Int64, Int64Type),
        DataType::Float64 => typed_option_or_null!(PlSmallStr::from_str(name), arr, f64, DataType::Float64, Float64Type),
        DataType::UInt64 => build_list_with_downcast!(PlSmallStr::from_str(name), arr, u64, DataType::UInt64, UInt64Type),
        DataType::String => {
            let mut builder = ListStringChunkedBuilder::new(PlSmallStr::from_str(name), len as usize, (len as usize) * 5);
            for idx in 0..len {
        ...
        DataType::Boolean => {
            let mut builder =
                ListBooleanChunkedBuilder::new(PlSmallStr::from_str(name), len as usize, (len as usize) * 5);
            for idx in 0..len {
            ...
            for idx in 0..len {
                ...
                match values {
                    Either::A(inner_arr) => {
                        let inner_len = inner_arr.len();
                        let mut inner_builder =
                            PrimitiveChunkedBuilder::<Int64Type>::new(PlSmallStr::from_str(name), inner_len as usize);
                        for inner_idx in 0..inner_len {
