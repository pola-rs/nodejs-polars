...
impl JsDataFrame {
    ...
    #[napi(catch_unwind)]
    pub fn unnest(&self, names: Vec<String>, separator: Option<String>) -> napi::Result<JsDataFrame> {
        let separator = separator.map_or(None, |s| Some(s));
        let df = self.df.unnest(names, separator.as_deref()).map_err(JsPolarsErr::from)?;
        Ok(df.into())
    }
    #[napi(catch_unwind)]
