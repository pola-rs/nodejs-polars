...
Micro-benchmarks comparing **nodejs-polars** `Series` against common JavaScript
alternatives for basic list operations:
...
| ------------- | ---------------------------------------- |
| `distinct`    | Return the unique values of a list       |
| `intsort`     | Sort a list of integers                  |
| `stringsort`  | Sort a list of strings                   |
| `intsum`      | Sum a list of integers                   |
| `valueCounts` | Count occurrences of each distinct value |

Each operation is run against list sizes of `1k`, `10k`, `100k`, and `1M`
elements, and each measurement is repeated **100 times** (see `numIterations` in
[`list-operations.js`](./list-operations.js)) to compute the mean, min, max, and
standard deviation.
...
```bash
# install the comparison libraries (lodash, ramda, chance)
yarn install

# make sure a build of nodejs-polars is available to `require("nodejs-polars")`
# (build it from the repo root first, e.g. `yarn build` in the project root)

# run the benchmark suite
node ./list-operations.js
```

Results are printed to the console with `console.table` and appended (as
headerless rows) to `./benchmarks/list-operations.csv`. Create the
`benchmarks/` directory first if it does not exist, or delete the CSV between
runs to start fresh.

To change the number of repetitions, edit the `config.numIterations` value in
[`list-operations.js`](./list-operations.js).

## How to read the results

- All timings are in **milliseconds** — lower is better.
- `mean` / `min` / `max` are the average, best, and worst run out of the 100
  iterations; `stddev` is the standard deviation across those runs.
- Rows are grouped by list size, then by operation, and sorted by mean time, so
  the fastest data structure for each operation appears first.

`series` is nodejs-polars. It is generally fastest for `intsum`, `distinct`, and
`stringsort`, with its advantage growing as the list size increases. The one
notable exception below is `valueCounts` at the `1k` size, where the fixed
overhead of crossing into the native layer outweighs the work — at larger sizes
`series` becomes the fastest option there as well.
...
- nodejs-polars version: 0.0.6

> **Note:** these numbers are from an early release (`0.0.6`). They are kept as a
> historical reference for the relative ordering between data structures. Absolute
> timings will differ on other hardware and newer versions — re-run the suite
